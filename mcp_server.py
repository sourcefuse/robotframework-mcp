...
@mcp.tool()
def start_browser(browser: str = "chrome", headless: bool = False, options: str = "") -> str:
    ...
    try:
        ...
        if browser not in valid_browsers:
            raise ValidationError(f"Browser must be one of: {', '.join(valid_browsers)}")
        
        # Parse additional options
        browser_args = []
        if options:
            # Split options by comma and clean them up
            browser_args = [arg.strip() for arg in options.split(',') if arg.strip()]
        ...
        # Create browser arguments section
        if browser_args:
            args_section = f"    Open Browser    ${{URL}}    {browser}    options=add_argument({';'.join(browser_args)})"
        else:
            args_section = f"    Open Browser    ${{URL}}    {browser}"
...
Start Browser Session
    [Documentation]    Start $browser_name browser with specified configuration
    [Tags]    browser    setup
    $browser_start_code
    Maximize Browser Window
    Set Selenium Timeout    $${TIMEOUT}
    Set Selenium Implicit Wait    10s
    Log    Browser started successfully: $browser_name
    [Teardown]    Close Browser

*** Keywords ***
Start Browser With Options
    [Documentation]    Start browser with custom configuration
...
Verify Browser Started
    [Documentation]    Verify browser is running and responsive
    $${title}=    Get Title
    Log    Browser is ready. Current title: $${title}
    $${url}=    Get Location
    Log    Current URL: $${url}
""")
        ...
        return template.substitute(
            ...
            browser_start_code=args_section
        )
        
    except ValidationError as e:
        return f"# VALIDATION ERROR: {str(e)}\n# Please provide a valid browser name: chrome, firefox, edge, or safari"
    except Exception as e:
        return f"# UNEXPECTED ERROR: {str(e)}\n# Please contact support."

@mcp.tool()
def navigate_to_url(url: str, wait_for_element: str = "", timeout: int = 30) -> str:
    ...
    try:
        ...
        # Create wait condition if element is specified
        wait_section = ""
        if wait_for_element:
            validated_selector = InputValidator.validate_selector(wait_for_element)
            wait_section = f"""
    # Wait for page to load completely
    Wait Until Page Contains Element    {validated_selector}    {timeout}s
    Wait Until Element Is Visible    {validated_selector}    {timeout}s"""
...
*** Test Cases ***
Navigate To Page
    [Documentation]    Navigate to $url and verify page loads
    [Tags]    navigation
    Go To    $${TARGET_URL}
    Wait Until Page Does Not Contain    Loading    timeout=$${TIMEOUT}$wait_condition
    $${title}=    Get Title
    Log    Successfully navigated to: $${TARGET_URL}
    Log    Page title: $${title}
    
*** Keywords ***
Navigate And Verify
    [Documentation]    Navigate to URL and verify page is ready
    [Arguments]    $${url}    $${expected_element}=
    Go To    $${url}
    Wait Until Page Does Not Contain    Loading    timeout=$${TIMEOUT}
    Run Keyword If    '$${expected_element}' != ''    
    ...    Wait Until Page Contains Element    $${expected_element}    $${TIMEOUT}
    $${title}=    Get Title
    $${current_url}=    Get Location
    Log    Navigation complete. Title: $${title}, URL: $${current_url}
    [Return]    $${current_url}
""")
...
@mcp.tool()
def interact_with_element(selector: str, action: str = "click", value: str = "", wait_timeout: int = 10) -> str:
    ...
    try:
        ...
        valid_actions = ["click", "input", "clear", "hover", "double_click", "right_click", "select", "check", "uncheck"]
        action = action.lower()
        ...
        if action == "click":
            action_code = f"    Click Element    {validated_selector}"
            action_description = f"Click on element: {validated_selector}"
        elif action == "input":
            if not value:
                raise ValidationError("Value is required for input action")
            action_code = f"    Input Text    {validated_selector}    {value}"
            action_description = f"Input text '{value}' into: {validated_selector}"
        elif action == "clear":
            action_code = f"    Clear Element Text    {validated_selector}"
            action_description = f"Clear text from: {validated_selector}"
        elif action == "hover":
            action_code = f"    Mouse Over    {validated_selector}"
            action_description = f"Hover over element: {validated_selector}"
        elif action == "double_click":
            action_code = f"    Double Click Element    {validated_selector}"
            action_description = f"Double click on: {validated_selector}"
        elif action == "right_click":
            action_code = f"    Open Context Menu    {validated_selector}"
            action_description = f"Right click on: {validated_selector}"
        elif action == "select":
            if not value:
                raise ValidationError("Value is required for select action")
            action_code = f"    Select From List By Label    {validated_selector}    {value}"
            action_description = f"Select '{value}' from dropdown: {validated_selector}"
        elif action == "check":
            action_code = f"    Select Checkbox    {validated_selector}"
            action_description = f"Check checkbox: {validated_selector}"
        elif action == "uncheck":
            action_code = f"    Unselect Checkbox    {validated_selector}"
            action_description = f"Uncheck checkbox: {validated_selector}"
...
$${ACTION_VALUE}        $input_value
$${WAIT_TIMEOUT}        $timeout

*** Test Cases ***
Interact With Element
    [Documentation]    $action_desc
    [Tags]    interaction    $action_name
    
    # Wait for element to be available
    Wait Until Page Contains Element    $${ELEMENT_SELECTOR}    $${WAIT_TIMEOUT}
    Wait Until Element Is Visible    $${ELEMENT_SELECTOR}    $${WAIT_TIMEOUT}
...
$action_implementation
    ...
    # Log the action
    Log    Action completed: $action_desc
    
*** Keywords ***
Wait For Element And Interact
    [Documentation]    Wait for element and perform specified action
    [Arguments]    $${selector}    $${action}    $${value}=
    
    # Enhanced waiting with retry logic
    Wait Until Keyword Succeeds    3x    2s    
    ...    Wait Until Element Is Visible    $${selector}    $${WAIT_TIMEOUT}
    ...
    Run Keyword If    '$${action}' == 'click'              Click Element    $${selector}
    ...    ELSE IF    '$${action}' == 'input'              Input Text    $${selector}    $${value}
    ...    ELSE IF    '$${action}' == 'clear'              Clear Element Text    $${selector}
    ...    ELSE IF    '$${action}' == 'hover'              Mouse Over    $${selector}
    ...    ELSE IF    '$${action}' == 'double_click'       Double Click Element    $${selector}
    ...    ELSE IF    '$${action}' == 'right_click'        Open Context Menu    $${selector}
    ...    ELSE IF    '$${action}' == 'select'             Select From List By Label    $${selector}    $${value}
    ...    ELSE IF    '$${action}' == 'check'              Select Checkbox    $${selector}
    ...    ELSE IF    '$${action}' == 'uncheck'            Unselect Checkbox    $${selector}
...
""")
        ...
        return template.substitute(
            ...
            action_name=action,
            action_implementation=action_code
        )
